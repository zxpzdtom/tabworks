(() => {
  ...
  var isExtensionPage = location.protocol === "chrome-extension:";
  var needsChromeMock = !isExtensionPage || !globalThis.chrome?.runtime?.connect;
  var isPreviewRuntime = needsChromeMock || !isExtensionPage && location.search.includes("preview=");
  var previewRecordingState = new URLSearchParams(location.search).get("recordingState");
  if (needsChromeMock) {
    globalThis.chrome = {
      runtime: {
        connect: () => ({
  ...
  var serviceMeta = document.getElementById("service-meta");
  var serviceTools = document.getElementById("service-tools");
  var serviceState = document.getElementById("service-state");
  var foregroundToggle = document.getElementById("foreground-toggle");
  var keepTabToggle = document.getElementById("keeptab-toggle");
  var logsBtn = document.getElementById("logs-btn");
  var guideBtn = document.getElementById("guide-btn");
  var sessionsEl = document.getElementById("sessions");
  var recorderDot = document.getElementById("recorder-dot");
  var recorderTitle = document.getElementById("recorder-title");
  var recorderState = document.getElementById("recorder-state");
  var recorderDetail = document.getElementById("recorder-detail");
  var recorderStartBtn = document.getElementById("recorder-start");
  var recorderStopBtn = document.getElementById("recorder-stop");
  ...
  function renderStatus(state = bridgeState) {
    ...
    if (bridgeState.connected) {
      setStatusVisual("connected", "已连接", "本地 bridge", "可以接收本地服务发来的浏览器自动化命令。");
    } else if (bridgeState.reconnecting) {
      setStatusVisual("connecting", "重连中", "正在等待 bridge 握手", "扩展已启用，本地服务恢复后会自动连接。");
    } else if (serviceOnline) {
      setStatusVisual("connecting", "服务在线", "本地 bridge", "HTTP 服务可用，但扩展 WebSocket 还未完成握手。");
    } else {
      setStatusVisual("disconnected", "未连接", "未检测到本地服务", "如果只想查看扩展能力，可以直接打开接口说明。");
    }
    ...
    serviceTools.querySelector("span").textContent = serviceOnline ? "日志服务已启动" : "日志服务未启动";
    logsBtn.disabled = !serviceOnline;
    logsBtn.textContent = serviceOnline ? "打开日志" : "日志离线";
    logsBtn.title = serviceOnline ? "打开本地日志服务" : "需要先启动 localhost:9527";
    serviceState.textContent = serviceOnline ? "日志服务已检测到" : "日志服务未检测到";
    renderRecording();
  }
  ...
  function shortSessionId(sessionId = "") {
    if (!sessionId)
      return "";
    return sessionId.length > 18 ? `${sessionId.slice(0, 11)}…${sessionId.slice(-4)}` : sessionId;
  }
  function renderRecording() {
    ...
    if (recording) {
      ...
      recorderDetail.textContent = `${shortSessionId(recording.sessionId)} · ${recording.title || recording.url || "当前标签页"} · ${recording.frameCount || 0} frames`;
      recorderSecondary.hidden = true;
      return;
    }
    if (lastRecordingResult) {
      if (lastRecordingResult.lastReplay) {
        ...
        recorderTitle.textContent = replay.failures?.length ? "回放完成，有步骤未命中" : "回放完成";
        recorderState.textContent = `${replay.played || 0} steps`;
        recorderDetail.textContent = replay.failures?.length ? `完成 ${replay.played || 0} 步，${replay.failures.length} 步未找到元素。` : `已刷新到录制起点，并回放 ${replay.played || 0} 个操作。`;
      } else {
        recorderTitle.textContent = "录制已保存";
        recorderState.textContent = `${lastRecordingResult.eventCount || 0} events`;
        recorderDetail.textContent = "回放会先刷新到录制起点，再按步骤执行。";
      }
    ...
    recorderDetail.textContent = "打开目标网页后，点开始录制；操作完成后回到这里停止。";
    recorderSecondary.hidden = true;
  }
  ...
  recorderStartBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      recording = null;
      recorderError = err instanceof Error ? err.message : "请刷新目标页面后重试。";
    }
    renderRecording();
  });
  recorderStopBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      recorderError = err instanceof Error ? err.message : "停止录制失败。";
    }
    renderRecording();
  });
  recorderReplayBtn.addEventListener("click", async () => {
    if (!lastRecordingResult?.sessionId)
      return;
  ...
  function orderedEvents(events = []) {
    return events.map((event, index) => ({ event, index, time: eventTime(event) })).sort((a, b) => {
...
`);
  ...
  function filenameSlug(value) {
    const normalized = String(value || "").normalize("NFKC").trim().replace(/[\\/:*?"<>|#%{}$!'@+`=&\u0000-\u001f]/g, " ").replace(/\s+/g, "-").replace(/^-+|-+$/g, "").slice(0, 60);
  ...
  var statusTimer = setInterval(checkViewerStatus, 3000);
  var sessionTimer = setInterval(() => port.postMessage({ type: "getSessions" }), 3000);
  var recordingTimer = setInterval(refreshRecordingStatus, 1500);
