(() => {
  ...
  function selectorMeta(el) {
    ...
    candidates.push(
      selectorCandidate(
        "role-name",
        role && aria ? `[role="${cssEscape(role)}"][aria-label="${cssEscape(aria)}"]` : "",
      ),
    );
    ...
    const preferred =
      selectors.find((item) => item.unique && item.kind !== "css") ||
      selectors.find((item) => item.unique) ||
      selectors[0] ||
      null;
  ...
  function targetElement(event) {
    ...
    const raw =
      ...
      event.target;
    if (!raw || raw.nodeType !== Node.ELEMENT_NODE) return null;
    return (
      raw.closest?.("button,a,input,textarea,select,[role],[data-testid],[contenteditable='true']") ||
      raw
    );
  }
  ...
  addCaptureListener("click", (event) => {
    if (takeEvent(event)) post("click", event);
  });
  addCaptureListener("dblclick", (event) => {
    if (takeEvent(event)) post("double-click", event);
  });
  ...
  addCaptureListener("change", (event) => {
    if (takeEvent(event)) post("change", event);
  });
})();
