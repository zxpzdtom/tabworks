(() => {
  // extension/src/recorder-content.ts
  (() => {
    const RECORDER_SCRIPT_VERSION = 13;
    if (globalThis.__tabworksRecorderVersion === RECORDER_SCRIPT_VERSION)
      return;
    ...
    function addCandidate(candidates, kind, selector) {
      if (!selector)
        return;
      const count = selector.includes(">>>") ? deepSelectorCount(selector) : selectorCount(selector);
      candidates.push({
        kind,
        selector,
        unique: count === 1,
    ...
    function selectorMeta(el) {
      ...
      if (testId)
        addCandidate(candidates, "testid", `[data-testid="${cssEscape(testId)}"]`);
      if (id)
        addCandidate(candidates, "id", `#${cssEscape(id)}`);
      if (ariaLabel)
        addCandidate(candidates, "aria", `[aria-label="${cssEscape(ariaLabel)}"]`);
      if (name)
        addCandidate(candidates, "name", `[name="${cssEscape(name)}"]`);
      addCandidate(candidates, "css", deepCssPath(el));
      addCandidate(candidates, "css-full", deepCssPath(el, 12));
      const preferred = candidates.find((item) => item.unique)?.selector || candidates.find((item) => item.kind === "css-full")?.selector || candidates.find((item) => item.selector)?.selector || "";
      return {
        tag: el.tagName,
        type: el.type || null,
        role,
    ...
    function frameContextForMessageSource(source, payload) {
      ...
      const context = {
        ...
        frameIndex: Number.isInteger(payloadContext.frameIndex) ? payloadContext.frameIndex : null,
    ...
    function sendReplayCursorRequestToParent(payload, timeoutMs = 1800) {
      ...
      const requestId = payload.requestId || `${Date.now()}-${++replayCursorSeq}-${Math.random().toString(36).slice(2, 7)}`;
      const message = {
        ...payload,
        [REPLAY_CURSOR_CHANNEL]: true,
        requestId,
    ...
    function send(event) {
      if (!recording || replaying)
        return;
      if (isDuplicateRecordedEvent(event))
        return;
    ...
    function isDuplicateRecordedEvent(event) {
      if (event.kind === "start" || event.kind === "stop")
        return false;
      const selector = event.element?.preferredSelector || event.element?.selectors?.find((item) => item.unique)?.selector || "";
      const key = [
        event.kind,
        selector,
      ...
      while (recentRecordedEvents.length && timestamp - recentRecordedEvents[0].timestamp > 180) {
        recentRecordedEvents.shift();
      }
      if (recentRecordedEvents.some((item) => item.key === key))
        return true;
      if (event.kind === "click" && recentRecordedEvents.some((item) => item.event?.kind === "click" && timestamp - item.timestamp <= 350 && closeNumber(item.event?.x, event.x, 12) && closeNumber(item.event?.y, event.y, 12))) {
        return true;
      }
    ...
    function syncRecordingState(afterStart) {
      ...
      chrome.runtime.sendMessage({ type: "tabworks-recording-sync" }, (response) => {
        ...
        if (recording && typeof afterStart === "function")
          afterStart();
      });
    }
    function inputValue(el) {
      ...
      if (el instanceof HTMLSelectElement) {
        return {
          value: el.multiple ? Array.from(el.selectedOptions).map((option) => option.value) : el.value,
          redacted: false
        };
      }
      if (el.isContentEditable)
        return { value: el.textContent || "", redacted: false };
    ...
    function targetElementFromEvent(event) {
      ...
      const el = raw?.nodeType === Node.ELEMENT_NODE ? raw : fallback?.nodeType === Node.ELEMENT_NODE ? fallback : null;
      return el?.closest?.("button,a,input,textarea,select,[role],[data-testid],[contenteditable='true']") || el || fallback;
    }
    ...
    function antCascaderMetaFromTarget(target, event) {
      ...
      if (!item)
        return null;
      const checkbox = item.querySelector(".ant-cascader-checkbox-inner") || item.querySelector(".ant-cascader-checkbox");
      ...
      return {
        ...
        clickedCheckbox: Boolean(checkboxRect) && Number.isFinite(Number(checkboxRect.left)) && Number(event?.clientX) >= checkboxRect.left - 8 && Number(event?.clientX) <= checkboxRect.right + 8 && Number(event?.clientY) >= checkboxRect.top - 8 && Number(event?.clientY) <= checkboxRect.bottom + 8
      };
    }
    function antSelectRemoveMetaFromTarget(target) {
      ...
      if (!item)
        return null;
      const content = item.querySelector(".ant-select-selection-item-content") || item;
      return {
        itemText: cleanLabel(content.innerText || content.textContent || ""),
        itemSelector: deepCssPath(item, 10),
    ...
    function cleanLabel(value) {
      return String(value || "").replace(/To pick up a draggable item[\s\S]*$/i, "").replace(/\s+/g, " ").trim();
    }
    ...
    function sortableSnapshotFromTarget(target) {
      ...
      while (row && row !== document.body) {
        ...
        if (rect && rect.width >= 120 && rect.height >= 28 && rect.height <= 90 && label && !/^(取\s*消|确\s*定|自定义导航栏)/.test(label)) {
          break;
        }
    ...
    function sortableAfter(before) {
      if (!before?.labels?.length || !before.sourceLabel)
        return null;
      ...
      for (const el of Array.from((scope || document).querySelectorAll("*"))) {
        if (rowLabel(el) === before.sourceLabel)
          matching.push(el);
      }
      ...
      if (!container)
        return null;
      const labels = Array.from(container.children).map(rowLabel).filter(Boolean);
    ...
    function recordMainBridgeEvent(payload) {
      if (!payload || replaying)
        return;
      if (payload.kind === "pointerdown" || payload.kind === "pointerup")
        return;
      if (!recording) {
      ...
      const meta = payload.element || selectorMeta(target?.closest?.("button,a,input,textarea,select,[role],[data-testid],[contenteditable='true']") || target);
      ...
      if (payload.kind === "pointerup") {
        if (!mainPointerStart || payload.pointerId !== mainPointerStart.pointerId)
          return;
        ...
        if (distance >= DRAG_DISTANCE_PX) {
          ...
          setTimeout(() => {
            ...
            send(sortable ? { ...dragEvent, sortable, source: "main-world", ...frameOverrides } : { ...dragEvent, source: "main-world", ...frameOverrides });
          }, 120);
    ...
    async function handleReplayCursorMessage(event, data) {
      ...
      try {
        ...
        postReplayCursorAck(target, data.requestId, false, err instanceof Error ? err.message : String(err));
      }
    }
    window.addEventListener("message", (event) => {
      ...
      if (!data || data[MAIN_EVENT_CHANNEL] !== true)
        return;
      ...
      if (!sameWindow && !data.fallbackToParent)
        return;
      if (!sameWindow && isChildFrame)
        return;
    ...
    addCaptureListener("pointerdown", (event) => {
      if (!takeEvent(event) || replaying || event.button !== 0 || !event.isPrimary)
        return;
      if (!recording) {
    ...
    addCaptureListener("pointermove", (event) => {
      if (!takeEvent(event) || !recording || replaying || !pointerStart || event.pointerId !== pointerStart.pointerId)
        return;
    ...
    addCaptureListener("contextmenu", (event) => {
      if (!takeEvent(event) || !recording || replaying)
        return;
      const meta = targetMetaFromEvent(event);
      if (!meta)
        return;
    ...
    addCaptureListener("keydown", (event) => {
      if (!takeEvent(event) || !recording || replaying || !shouldRecordKey(event))
        return;
      send({
        kind: "key",
        key: event.key,
    ...
    addCaptureListener("input", (event) => {
      if (!takeEvent(event) || replaying || !isInputLike(event.target))
        return;
    ...
    function selectorForEvent(event) {
      ...
      return selectors.find((item) => item.unique)?.selector || event?.element?.preferredSelector || selectors.find((item) => item.selector)?.selector || "";
    }
    function selectorForSortableEvent(event) {
      const selectors = event?.sortable?.rowElement?.selectors || [];
      return selectors.find((item) => item.unique)?.selector || event?.sortable?.rowElement?.preferredSelector || "";
    }
    ...
    function sameSortableMove(a, b) {
      ...
      if (a.sortable.sourceLabel || b.sortable.sourceLabel || a.sortable.rowElement || b.sortable.rowElement) {
        ...
        const sameRowSelector = selectorForEvent(a) && selectorForEvent(a) === selectorForEvent(b);
...
[data-tabworks-toast].show {
  ...
  transform: translate3d(0, 0, 0) scale(1);
    ...
    function trustedReplayClick(point) {
      ...
      if (!topPoint)
        return Promise.resolve({ ok: false, error: "invalid point" });
      return new Promise((resolve) => {
        chrome.runtime.sendMessage({
    ...
    function isVisibleElement(el) {
      ...
      return rect.width > 0 && rect.height > 0 && style.display !== "none" && style.visibility !== "hidden" && !el.closest?.(".ant-select-dropdown-hidden");
    }
    function findAntCascaderClickTarget(event) {
      const meta = event?.antCascader;
      if (!meta?.clickedCheckbox)
        return null;
      const candidates = [
      ...
      const menus = Array.from(document.querySelectorAll(".ant-cascader-menu-item")).filter(isVisibleElement);
      if (meta.itemText) {
      ...
      for (const candidate of candidates) {
        const checkbox = candidate.matches?.(".ant-cascader-checkbox-inner, .ant-cascader-checkbox") ? candidate : candidate.querySelector?.(".ant-cascader-checkbox-inner") || candidate.querySelector?.(".ant-cascader-checkbox");
    ...
    function findAntSelectRemoveTarget(event) {
      ...
      const items = Array.from(document.querySelectorAll(".ant-select-selection-item")).filter(isVisibleElement);
    ...
    function replayClickTarget(el) {
      ...
      return {
        el: el?.closest?.("button,a,label,input,textarea,select,[role='button'],[role='option'],[role='menuitem'],[role='menuitemcheckbox'],.ant-cascader-menu-item,.ant-select-item-option") || el,
        mode: "default"
      };
    }
    ...
    function pointForReplayClick(el, event) {
      ...
      if (Number.isFinite(offsetX) && Number.isFinite(offsetY)) {
        const scaleX = Number.isFinite(recordedWidth) && recordedWidth > 0 ? rect.width / recordedWidth : 1;
        const scaleY = Number.isFinite(recordedHeight) && recordedHeight > 0 ? rect.height / recordedHeight : 1;
        return {
    ...
    async function replayPointerGesture(event, type) {
      ...
      if (!el)
        return { ok: false, kind: event.kind, selector, error: "元素未找到" };
    ...
    async function replayEvent(event) {
      if (event.kind === "click") {
        const selector = selectorForEvent(event);
        let target = await waitForAntSelectRemoveTarget(event) || await waitForAntCascaderClickTarget(event);
        ...
        const point = target.mode === "ant-cascader-checkbox" || target.mode === "ant-select-remove" ? centerOf(target.el) : pointForReplayClick(target.el, event);
        await moveReplayCursor(point);
        await pulseReplayCursor(point);
        if (target.mode === "ant-select" || target.mode === "ant-cascader" || target.mode === "ant-cascader-checkbox" || target.mode === "ant-select-remove") {
      ...
      if (event.kind === "long-press")
        return replayPointerGesture(event, "long-press");
      if (event.kind === "drag")
        return replayPointerGesture(event, "drag");
      if (event.kind === "key") {
      ...
      if (event.kind === "submit") {
        ...
        const el = findEventElement(event);
        if (!el)
          return { ok: false, kind: event.kind, selector, error: "表单未找到" };
        el.scrollIntoView({ block: "center", inline: "center" });
        await wait(80);
    ...
    function normalizePlayableEvents(events) {
      const playable = events.filter((event) => ["click", "input", "scroll", "submit", "long-press", "drag", "double-click", "context-menu", "key"].includes(event.kind));
      const normalized = [];
      for (const event of playable) {
        const previous = normalized[normalized.length - 1];
        if (normalized.slice(-8).some((item) => isDuplicateDragEvent(item, event) || isDuplicateClickEvent(item, event))) {
          continue;
        }
        if (event.kind === "double-click" && previous?.kind === "click" && selectorForEvent(previous) === selectorForEvent(event)) {
          normalized.pop();
        ...
        if (event.kind === "input" && previous?.kind === "input" && selectorForEvent(previous) === selectorForEvent(event) && previous.value === event.value) {
    ...
    chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
      ...
      if (message?.type === "tabworks-recording-replay") {
        replayEvents(message.events || [], message.options || {}).then((result) => sendResponse(result)).catch((err) => sendResponse({
          ok: false,
